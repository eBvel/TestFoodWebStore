...
from tests.test_data import headers, cart_data
...
class TestCartPage:
    ...
    @mark.parametrize('test_product', ['sandwich'], indirect=True)
    def test_calculation_total_cost(self, test_product, auth_by_user1):
        self.catalog.open()
        self.catalog.add_product(test_product.name, 3)

        self.cart.open()
        #EXPECTED_VALUE
        self.cart.check_total_cost(
            f'Итого: {(test_product.price * 3):.{0}f} ₽'
        )
